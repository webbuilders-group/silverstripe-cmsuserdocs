<div class="cms-content center cmsdocviewer-content cmsdocviewer-results-content" data-layout-type="border" data-pjax-fragment="CurrentForm">
    <div class="cms-content-fields center cms-panel-padded ui-widget" data-layout-type="border">
        <% if $Results %>
            <p class="intro"><%t CMSDocumentationViewer.SEARCH_RESULTS "_Your search for <strong>&quot;{query}&quot;</strong> found {total} result(s). Showing page {current_page} of {total_pages}." query=$Query.XML total=$TotalResults current_page=$ThisPage total_pages=$TotalPages %></p>
            ...
            <% if $SearchPages %>
                <ul class="pagination">
                    <% if $PrevUrl %>
                        <li class="prev"><a href="$PrevUrl" class="cms-panel-link" data-pjax-target="CurrentForm"><%t CMSDocumentationViewer.PREV "_Prev" %></a></li>
                    <% end_if %>
                    ...
                    <% if $NextUrl %>
                        <li class="next"><a href="$NextUrl" class="cms-panel-link" data-pjax-target="CurrentForm"><%t CMSDocumentationViewer.NEXT "_Next" %></a></li>
                    <% end_if %>
                </ul>
            <% end_if %>
        <% else %>
